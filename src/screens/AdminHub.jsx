...
const Btn = ({ children, onClick, variant="primary", size="md", full=false, disabled=false }) => {
  ...
  return <button onClick={onClick} disabled={disabled}
    ...
    style={{ padding:pad, background:s.bg, color:s.c, border:s.b||"none",
      ...
      justifyContent:"center", gap:8, borderRadius:"var(--r-sm)", transition:"background .18s, transform .1s",
      ...
      {type==="textarea"
        ? <textarea value={value} onChange={onChange} placeholder={placeholder} rows={rows}
            ...
            style={{ width:"100%", padding:"11px 14px", border:`1.5px solid ${error?"var(--red)":foc?"var(--amber)":"var(--rule)"}`,
              ...
              fontSize:14, color:"var(--ink)", outline:"none", borderRadius:"var(--r-sm)", resize:"vertical", lineHeight:1.6 }}/>
        : <input type={type} value={value} onChange={onChange} placeholder={placeholder}
            onFocus={()=>setFoc(true)} onBlur={()=>setFoc(false)}
            style={{ width:"100%", padding:"11px 14px", border:`1.5px solid ${error?"var(--red)":foc?"var(--amber)":"var(--rule)"}`,
              background:foc?"var(--surface)":"var(--cream)", fontFamily:"var(--font-sans)",
              fontSize:14, color:"var(--ink)", outline:"none", borderRadius:"var(--r-sm)" }}/>
      }
...
export default function AdminHub({ onNavigate, toast: parentToast }) {
